...
-- Новий розділ «Бургери та шаурма». Верхній засів уже не спрацює —
-- він стоїть під умовою «якщо розділів ще немає», — тож додаємо окремо.
update public.items set sort_order = 9
...
insert into public.items (site_id, collection, title, extra, sort_order)
select 5, 'cats', 'Бургери та шаурма', '{"catkey":"burg"}'::jsonb, 8
where not exists (
  select 1 from public.items
  where site_id = 5 and collection = 'cats' and extra->>'catkey' = 'burg');

-- Бургери й шаурму виносимо з «Фірмових» у власний розділ,
...
where site_id = 5 and collection = 'menu'
  ...
  and (title like '%ургер%' or title like '%аурма%');
