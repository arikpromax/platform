        ...
        >
          ...
          <div className="row__actions">
            {col.rowToggle && (() => {
              ...
              return (
                <button
                  className={"btn btn--sm rowtog" + (on ? " rowtog--on" : " btn--ghost")}
                  title={t.title ?? t.label}
                  disabled={busy || !canEdit}
                  onClick={() => togglePick(item, t.flag, !on)}
                >
                  {on ? "✓ " : "+ "}
                  {t.label}
                </button>
      ...
      {helpOpen && (
        <div className="cropper-veil" onClick={(e) => e.target === e.currentTarget && setHelpOpen(false)}>
          <div className="help" role="dialog" aria-label="Як користуватися адмінкою">
            ...
            <p>Кнопка <span className="kbd">+ банер</span> просто в рядку списку страв або сетів — і позиція покажеться великою карткою вгорі головної. Назву, фото, ціну й акцію банер бере з самої позиції. Натиснути ще раз — прибрати з банера.</p>
            <p>Порядок банерів задається числом: <span className="kbd">Редагувати</span> → поле <span className="kbd">Яким показувати в банері</span>. 1 — перший, 2 — другий. Нумерація спільна для страв і сетів; хто без числа — стане в кінець.</p>
